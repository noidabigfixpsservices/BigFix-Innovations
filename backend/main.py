...
async def export_bes(req: BesExportRequest):
    try:
        ...
        rel = req.relevance.strip() or "true"
        act = req.actionscript.strip() or ""
        date_str = datetime.now().strftime('%Y-%m-%d')
        
        # YOUR exact XML string - untouched!
        ...
        # --- NEW NATIVE DESKTOP SAVING LOGIC ---
        # 1. Find the user's Downloads folder
        downloads_folder = str(Path.home() / "Downloads")
        
        # 2. Make the filename safe (replace spaces with underscores, remove weird characters)
        safe_filename = "".join(c for c in title if c.isalnum() or c in " _-").strip().replace(" ", "_") + ".bes"
        file_path = os.path.join(downloads_folder, safe_filename)

        # 3. Write directly to the hard drive
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(xml_content)
            
        return {"success": True, "message": f"Fixlet saved successfully to:\n{file_path}"}
